            ...
            {myStored.length > 0 && (
              <div className="glass-card rounded-2xl p-5 sm:p-8">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg sm:text-xl font-semibold font-display">My Requests</h3>
                ...
                {myBookings.length === 0 && !loadingMine ? (
                  ...
                  <div className="space-y-3">
                    {myBookings.map((b) => {
                      ...
                      return (
                        <div key={b.id} className="rounded-xl border border-border bg-secondary/40 p-3 sm:p-4">
                          <div className="flex items-start justify-between gap-2 mb-2 flex-wrap">
                            <div className="min-w-0 flex-1">
                              <p className="text-sm font-medium text-foreground break-words">{b.topic || "Session"}</p>
          ...
          <div className="glass-card rounded-2xl p-5 sm:p-8" id="book">
            {success ? (
              <div className="text-center py-6 sm:py-8">
                ...
                <h3 className="font-display text-xl sm:text-2xl font-bold mb-2">Request sent!</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                ...
                <div id="book-slot" className="pt-2 border-t border-border space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground break-words">Times shown in your timezone: {tz}</p>

                  <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1 snap-x scroll-smooth">
                    {days.map((d) => {
                      const isSel = d.getTime() === selectedDay.getTime();
                      return (
                        <button key={d.toISOString()} type="button" onClick={() => setSelectedDay(d)}
                          className={`flex-shrink-0 snap-start px-3 py-2 rounded-lg text-xs font-medium transition-all border ${
                      ...
                      {slots.map((s) => {
                        ...
                        return (
                          <button key={s.toISOString()} type="button" disabled={taken} onClick={() => setSelectedSlot(s)}
                            className={`px-2 py-2 rounded-lg text-xs font-medium transition-all border min-w-0 ${
                  ...
                  {selectedSlot && (
                    <div className="rounded-lg bg-primary/10 border border-primary/30 px-3 py-2 text-xs sm:text-sm break-words">
